...
" ui-widget-header ui-corner-all";}else{this.range.removeClass("ui-slider-range-min ui-slider-range-max").css({"left":"","bottom":""});}
...
this.element.removeClass("ui-tabs ui-widget ui-widget-content ui-corner-all ui-tabs-collapsible");this.tablist.removeClass("ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all").removeAttr("role");this.anchors.removeClass("ui-tabs-anchor").removeAttr("role").removeAttr("tabIndex").removeUniqueId();this.tablist.unbind(this.eventNamespace);this.tabs.add(this.panels).each(function(){if($.data(this,"ui-tabs-destroy")){$(this).remove();}else{$(this).removeClass("ui-state-default ui-state-active ui-state-disabled "+
...
$(function(){var cache={};$("#autocomplete").autocomplete({minLength:3,appendTo:'.search-box',source:function(request,response){var term=request.term;if(term in cache){response(cache[term]);return;}
$.getJSON("//cdn.1337x.to/cdnsuggest.php",request,function(data,status,xhr){cache[term]=data.data;response(data.data);});},select:function(event,ui){}}).data('uiAutocomplete')._renderItem=function(ul,item){var srchTerm=this.term.trim().split(/\s+/).join('|');var strNewLabel=item.label;regexp=new RegExp('('+srchTerm+')',"ig");var strNewLabel=strNewLabel.replace(regexp,"<span class='highlight-class'>$1</span>");return $("<li></li>").data("item.autocomplete",item).append("<a>"+strNewLabel+"</a>").appendTo(ul);};});
...
wrapper.append('<div class="trigger"><span class="tip"></span>');if(!(isiOS&&!settings.forceiOS)){wrapper.append('<ul class="options">');}
...
var tablet_width=768;function list_box(){if($(window).width()<tablet_width){$('.list-box h2').addClass('mobile');}else{$('.list-box h2').removeClass('mobile');}}
$(window).resize(function(){list_box();});$(document).ready(function(){list_box();var top_bar=$('.top-bar .container').html();var navigation_html=$('nav').html();$('.mobile-menu').html(navigation_html+top_bar);$('.top-bar-left').html(navigation_html);$('.navbar-menu').click(function(){$('.mobile-menu').fadeToggle();return false;});$('.list-box').on('click','h2.mobile',function(){$(this).toggleClass('active');$(this).next('ul').slideToggle();return false;});$('.table-list td.name').each(function(){comments_width=$(this).find('.comments').width();$(this).find('.comments').parent('td.name').css('padding-right',50+comments_width);});$('a.scroll-top').click(function(){$('html,body').stop(true,true).animate({scrollTop:$('html,body').offset().top-50},550);return false;});$('a[data-toggle="dropdown"]').click(function(){$(this).next('.dropdown-menu').slideToggle("slow",function(){$(this).parents('.torrent-category-detail').toggleClass('js-active-button');});return false;});$('form').attr('novalidate',true);$("a.print").click(function(){window.print();return false;});});

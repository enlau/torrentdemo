...
|--------------------------------------------------------------------------
| Intentionally prefixed with "moviese_" so they do not collide with
| helpers from other root section files.
|--------------------------------------------------------------------------
*/
if (!function_exists('moviese_request_uri')) {
    function moviese_request_uri()
    {
        return $_SERVER['REQUEST_URI'] ?? '';
    }
}

if (!function_exists('moviese_request_path')) {
    function moviese_request_path()
    {
        $path = parse_url(moviese_request_uri(), PHP_URL_PATH);
        return is_string($path) ? $path : '';
    }
}

if (!function_exists('moviese_path_parts')) {
    function moviese_path_parts()
    {
        $path = trim(moviese_request_path(), '/');
...
if (!function_exists('moviese_last_segment')) {
    function moviese_last_segment()
    {
        $parts = moviese_path_parts();
...
/*
|--------------------------------------------------------------------------
| Missing file reader helper (this was causing your fatal error)
|--------------------------------------------------------------------------
*/
if (!function_exists('rf_moviese')) {
    function rf_moviese($file)
    {
        $contents = '';
...
function set_default_moviese()
{
    ...
    $kw = get_moviese_keywords();

    for ($i = 0; $i < 28; $i++) {
        $a1 = "movieslink$i";
        $a2 = "moviestlink$i";
        $tpl->assign($a1, '/movies/' . str_replace(' ', '-', $kw[$i] ?? '') . '.html');
        $tpl->assign($a2, $kw[$i] ?? '');
    }
...
function show_archive_moviese_old()
{
    global $tpl, $url, $moviese_file;

    $tpl->assign('title', 'Torrent Archive');

    $contents = rf_moviese($moviese_file);
    $kw = explode("\n", $contents);
...
function show_archive_moviese()
{
    global $tpl, $url, $moviese_file;

    $segment = moviese_last_segment();

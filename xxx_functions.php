...
|--------------------------------------------------------------------------
| Intentionally prefixed with "xxxe_" so they do not collide with
| helpers from other root section files.
|--------------------------------------------------------------------------
*/
if (!function_exists('xxxe_request_uri')) {
    function xxxe_request_uri()
    {
        return $_SERVER['REQUEST_URI'] ?? '';
    }
}

if (!function_exists('xxxe_request_path')) {
    function xxxe_request_path()
    {
        $path = parse_url(xxxe_request_uri(), PHP_URL_PATH);
        return is_string($path) ? $path : '';
    }
}

if (!function_exists('xxxe_path_parts')) {
    function xxxe_path_parts()
    {
        $path = trim(xxxe_request_path(), '/');
...
if (!function_exists('xxxe_last_segment')) {
    function xxxe_last_segment()
    {
        $parts = xxxe_path_parts();
...
function set_default_xxxe()
{
    ...
    $kw = get_xxxe_keywords();

    for ($i = 0; $i < 28; $i++) {
        $a1 = "xxxlink$i";
        $a2 = "xxxtlink$i";
        $tpl->assign($a1, '/xxx/' . str_replace(' ', '-', $kw[$i] ?? '') . '.html');
        $tpl->assign($a2, $kw[$i] ?? '');
    }
...
function show_archive_xxxe_old()
{
    global $tpl, $url, $xxxe_file;

    $tpl->assign('title', 'Torrent Archive');

    $contents = rf_xxxe($xxxe_file);
    $kw = explode("\n", $contents);
...
function show_archive_xxxe()
{
    global $tpl, $url, $xxxe_file;

    $segment = xxxe_last_segment();
...
function rss_fetch_content_xxxe($url)
{
...
function rss_to_array_xxxe($tag, $array, $url)
{
    $rss_array = array();

    $rssContent = rss_fetch_content_xxxe($url);
...
function cleanRss_xxxe($targetrss, $targetvar)
{
...
$rss_array = rss_to_array_xxxe($rss_item_tag, $rss_tags, $xxxmainRSS_url);

for ($rssparser = 0; $rssparser < 26; $rssparser++) {
    $title = $rss_array[$rssparser]['title'] ?? '';

    if ($title === '') {
        $tpl->assign('xxxrsstorrentlink' . $rssparser, '');
        $tpl->assign('xxxrsstorrent' . $rssparser, '');
        continue;
    }

    $cleanTitle = cleanRss_xxxe($title, $title);

    $tpl->assign('xxxrsstorrentlink' . $rssparser, '/xxx/' . $cleanTitle);
    $tpl->assign('xxxrsstorrent' . $rssparser, $cleanTitle);
}

...
|--------------------------------------------------------------------------
| These are intentionally prefixed with "gamese_" so they do not collide
| with helpers from other root section files.
|--------------------------------------------------------------------------
*/
if (!function_exists('gamese_request_uri')) {
    function gamese_request_uri()
    {
        return $_SERVER['REQUEST_URI'] ?? '';
    }
}

if (!function_exists('gamese_request_path')) {
    function gamese_request_path()
    {
        $path = parse_url(gamese_request_uri(), PHP_URL_PATH);
        return is_string($path) ? $path : '';
    }
}

if (!function_exists('gamese_path_parts')) {
    function gamese_path_parts()
    {
        $path = trim(gamese_request_path(), '/');
...
if (!function_exists('gamese_last_segment')) {
    function gamese_last_segment()
    {
        $parts = gamese_path_parts();
...
function show_archive_gamese_old()
{
    global $tpl, $url, $gamese_file;

    $tpl->assign('title', 'Torrent Archive');

    $contents = rf_gamese($gamese_file);
    $kw = explode("\n", $contents);
...
function show_archive_gamese()
{
    global $tpl, $url, $gamese_file;

    $segment = gamese_last_segment();
...
function rss_fetch_content_gamese($url)
{
...
function rss_to_array_gamese($tag, $array, $url)
{
    $rss_array = array();

    $rssContent = rss_fetch_content_gamese($url);
...
function cleanRss_gamese($targetrss, $targetvar)
{
...
$rss_array = rss_to_array_gamese($rss_item_tag, $rss_tags, $gamesmainRSS_url);

for ($rssparser = 0; $rssparser < 26; $rssparser++) {
    $title = $rss_array[$rssparser]['title'] ?? '';

    if ($title === '') {
        $tpl->assign('gamesrsstorrentlink' . $rssparser, '');
        $tpl->assign('gamesrsstorrent' . $rssparser, '');
        continue;
    }

    $cleanTitle = cleanRss_gamese($title, $title);

    $tpl->assign('gamesrsstorrentlink' . $rssparser, '/games/' . $cleanTitle);
    $tpl->assign('gamesrsstorrent' . $rssparser, $cleanTitle);
}
